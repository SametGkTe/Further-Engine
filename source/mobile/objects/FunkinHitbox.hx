...
class FunkinHitbox extends Hitbox {
	...
	public function new(?mode:String, ?showHints:Bool = true, ?globalAlpha:Float = 0.7):Void
	{
		super(mode, false);
		...
		else
		{
			...
			for (buttonData in currentHint)
			{
				...
				if (addButton)
					addHint(buttonName, buttonIDs, buttonUniqueID, buttonX, buttonY, buttonWidth, buttonHeight, CoolUtil.colorFromString(buttonColor), buttonReturn);
			}
		}
	...
	override public function createHint(name:Array<String>, uniqueID:Int, x:Float, y:Float, width:Int, height:Int, color:Int = 0xFFFFFF, ?returned:String):MobileButton
	{
		...
		var VSliceAllowed:Bool = (currentMode == 'V Slice' && 4 < 10);
